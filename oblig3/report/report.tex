...
When solving eq.~(\ref{eq:dimless}) numerically, the constants $\alpha$, $t_c$ and $m_c$ must be chosen so that the results can be interpreted physically.

\subsection{Runge-Kutta methods}
The Runge-Kutta methods are based on predictor-corrector methods, where the slopes in intermediate steps are used to solve 
