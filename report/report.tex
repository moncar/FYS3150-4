...
The choice of orthogonal polynomial reflects both the integration range, and the form of the integral -- it is useful to consider a rewrite where the integrand $f(x) \to W(x) g(x)$ with $W(x)$ the \textit{weight function} that project the function $g(x)$ onto the chosen orthogonal basis of polynomials. 

In the case where $W(x)$ can be singled out, the Gaussian quadrature consists of evaluating $g(x_i)$ at the zeros of the chosen orthogonal polynomial multiplied by the weights $w_i$, who also are determined by the polynomial.
