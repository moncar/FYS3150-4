...
\[ \int f(x) \diff x \approx \int Q_{N-1}(x) \diff x. \]

The numerical approximation gives that
\[ \int Q_{N-1}(x) \diff x \approx \sum_i Q_{N-1}(x_i) w_i \]
where the weights can be determined by
